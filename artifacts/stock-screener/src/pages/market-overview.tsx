...
import {
  ...
  getGetTopGainersQueryKey,
  getGetTopLosersQueryKey,
  getGetMostActiveQueryKey,
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={cn(
            "flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-mono font-medium",
            isUp
...
function StockRow({
  ...
  return (
    <Link
      ...
      className="group flex items-center justify-between gap-4 rounded-md px-3 py-2.5 transition-colors hover:bg-muted/60"
    >
      ...
      <div className="text-right">
        ...
        <span
          className={cn(
            "inline-block rounded px-1.5 py-0.5 text-xs font-mono font-medium tabular-nums",
            changeBg(changePercent),
          )}
        >
          {formatChangePercent(changePercent)}
        </span>
...
export default function MarketOverview() {
  ...
  const { data: gainersData, isLoading: gainersLoading } = useGetTopGainers(
    ...
    {
      query: {
        queryKey: getGetTopGainersQueryKey({ exchange: "NSE", limit: 8 }),
        refetchInterval: 60_000,
      },
    },
  );
  const { data: losersData, isLoading: losersLoading } = useGetTopLosers(
    ...
    {
      query: {
        queryKey: getGetTopLosersQueryKey({ exchange: "NSE", limit: 8 }),
        refetchInterval: 60_000,
      },
    },
  );
  const { data: activeData, isLoading: activeLoading } = useGetMostActive(
    ...
    {
      query: {
        queryKey: getGetMostActiveQueryKey({ exchange: "NSE", limit: 8 }),
        refetchInterval: 60_000,
      },
    },
  );
    ...
    <div className="mx-auto max-w-screen-2xl px-4 py-6 lg:px-6">
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Indian equity markets — NSE &amp; BSE. Prices refresh every 60s.
        </p>
      </div>
        ...
        <Section title="Top Gainers" icon={TrendingUp} loading={gainersLoading}>
          {gainersLoading
            ? Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-12 mx-3 my-1.5 animate-pulse rounded bg-muted/40"
                />
              ))
