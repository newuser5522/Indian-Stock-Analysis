...
import {
  ...
  type GetStockHistoryPeriod,
  type GetStockHistoryInterval,
...
function calculateRSI(closes: number[], period = 14): (number | null)[] {
  ...
  rsi.push(
    parseFloat((100 - 100 / (1 + avgGain / (avgLoss || 0.001))).toFixed(2)),
  );
...
function PriceTooltip({
  ...
  return (
    <div className="rounded-md border bg-popover px-3 py-2 text-xs shadow-lg space-y-1">
      ...
      {payload.map((p) => (
        <p key={p.dataKey} className="font-mono font-bold">
          {formatPrice(p.value)}
        </p>
      ))}
    </div>
...
export default function StockDetail() {
  ...
  const [period, setPeriod] = useState<GetStockHistoryPeriod>("1mo");
  const [interval, setIntervalStr] = useState<GetStockHistoryInterval>("1d");
  ...
  const { data: historyData, isLoading: histLoading } = useGetStockHistory(
    symbol,
    { period, interval },
    {
      query: {
        queryKey: getGetStockHistoryQueryKey(symbol, { period, interval }),
        enabled: !!symbol,
      },
    },
  );
  ...
  const isWatched = (watchlistData?.items ?? []).some(
    (i) => i.symbol === symbol,
  );
  ...
  const latestRsi =
    chartData.length > 0
      ? [...chartData].reverse().find((d) => d.rsi != null)?.rsi
      : null;
  const rsiColor =
    latestRsi != null
      ? latestRsi >= 70
              ...
              <>
                ...
                {(quote?.sector || fundamentals?.sector) && (
                  <p className="text-xs text-muted-foreground/70 mt-0.5">
                    ...
                    {(fundamentals?.industry ?? quote?.industry)
                      ? ` · ${fundamentals?.industry ?? quote?.industry}`
                      : ""}
                  </p>
                )}
        ...
        <Button
          ...
          onClick={() => {
            if (isWatched)
              ...
              addMutation.mutate({
                data: { symbol, name: quote.name, exchange: quote.exchange },
              });
          }}
        >
          <Star
          ...
          <div className="mb-6 flex flex-wrap items-end gap-4">
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <span
                  className={cn(
                    "rounded-full px-2.5 py-0.5 font-mono text-sm font-medium tabular-nums",
                    changeBg(quote.changePercent),
                  )}
                >
            ...
            <div className="flex flex-wrap gap-5 text-xs text-muted-foreground pb-1">
              ...
              <div>
                <span>High</span>
                <p className="font-mono font-medium text-emerald-400 tabular-nums">
                  {formatPrice(quote.high)}
                </p>
              </div>
              <div>
                <span>Low</span>
                <p className="font-mono font-medium text-red-400 tabular-nums">
            ...
            {chartChange != null && (
              <span
                className={cn(
                  ...
                  chartChange >= 0 ? "text-emerald-400" : "text-red-400",
                )}
              >
                {chartChange >= 0 ? "+" : ""}
                {chartChange.toFixed(2)}% ({period})
              </span>
            )}
            {latestRsi != null && (
        ...
        {histLoading ? (
          ...
          <div>
            ...
            <div className="mt-1 border-t border-border/50 pt-2">
              <p className="text-[10px] font-mono text-muted-foreground mb-1 px-1">
                RSI (14)
              </p>
      ...
      {fundamentals ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <FundSection title="Valuation">
            <FundRow
              label="P/E (Trailing)"
              value={formatRatio(fundamentals.pe)}
            />
            <FundRow
              label="P/E (Forward)"
            ...
            <FundRow
              label="EPS (TTM)"
              value={
                fundamentals.eps != null ? formatPrice(fundamentals.eps) : "—"
              }
            />
            <FundRow
              label="EPS Growth"
              value={formatPercent(fundamentals.epsGrowth)}
              highlight={
                fundamentals.epsGrowth != null
                  ? fundamentals.epsGrowth > 0
                    ? "positive"
                    : "negative"
                  : null
              }
          ...
          <FundSection title="Profitability">
            ...
            <FundRow
              label="Gross Margin"
              value={formatPercent(fundamentals.grossMargins)}
            />
            <FundRow
          ...
          <FundSection title="Financial Health">
            ...
            <FundRow
              label="Quick Ratio"
              value={formatRatio(fundamentals.quickRatio)}
            />
            <FundRow
              label="Debt/Equity"
            ...
            <FundRow
              label="Free Cashflow"
              value={formatLargeNumber(fundamentals.freeCashflow)}
            />
            <FundRow
              label="Operating CF"
              value={formatLargeNumber(fundamentals.operatingCashflow)}
            />
            <FundRow
              label="EBITDA"
              value={formatLargeNumber(fundamentals.ebitda)}
            />
          </FundSection>
          ...
          <FundSection title="Market & Trading">
            ...
            <FundRow
              label="VWAP"
              value={
                fundamentals.vwap != null ? formatPrice(fundamentals.vwap) : "—"
              }
            />
            <FundRow
              label="Face Value"
              value={
                fundamentals.faceValue != null
            ...
            <FundRow
              label="Delivery %"
              value={
                fundamentals.deliveryPct != null
                  ? `${fundamentals.deliveryPct.toFixed(2)}%`
                  : "—"
              }
              highlight={
                fundamentals.deliveryPct != null
                  ? fundamentals.deliveryPct > 50
                    ? "positive"
                    : null
                  : null
              }
            ...
            <FundRow
              ...
              value={
                fundamentals.annualVolatility != null
                  ? `${fundamentals.annualVolatility}%`
                  : "—"
              }
            />
            <FundRow
            ...
            {fundamentals.analystRating && (
              <FundRow
                ...
                value={fundamentals.analystRating.toUpperCase()}
                highlight={
                  fundamentals.analystRating.includes("buy")
                    ? "positive"
                    : fundamentals.analystRating.includes("sell")
                      ? "negative"
                      : null
                }
              />
            ...
            {fundamentals.targetPrice != null && (
              <FundRow
                label="Target Price"
                value={formatPrice(fundamentals.targetPrice)}
              />
            )}
          </FundSection>
        </div>
