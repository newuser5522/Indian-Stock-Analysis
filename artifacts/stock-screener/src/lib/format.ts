...
export function formatIndianCurrency(value: number | undefined | null): string {
  if (value == null) return "—";
  return new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 2, minimumFractionDigits: 2 }).format(value);
}

export function formatIndianNumber(value: number | undefined | null): string {
  if (value == null) return "—";
...
export function getPriceColorClass(change: number | undefined | null): string {
  return changeColor(change);
}

export function getPriceBgColorClass(change: number | undefined | null): string {
  return changeBg(change);
}
