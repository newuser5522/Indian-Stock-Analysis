...
import { fetchQuotes, fetchSummary, fetchHistory, searchYahoo } from "../lib/yahoo-finance";
...
router.get("/stocks/fundamentals/:symbol", async (req, res): Promise<void> => {
  try {
    ...
    res.json(
      GetStockFundamentalsResponse.parse({
        symbol,
        name: price.shortName ?? price.longName ?? info?.name ?? symbol,
        sector: profile.sector ?? info?.sector,
        industry: profile.industry,
        marketCap: det.marketCap?.raw ?? quote?.marketCap,
        enterpriseValue: det.enterpriseValue?.raw ?? kstat.enterpriseValue?.raw,
        pe: det.trailingPE?.raw ?? quote?.trailingPE,
        ...
        eps: kstat.trailingEps?.raw ?? quote?.trailingEps,
        ...
        sharesOutstanding: kstat.sharesOutstanding?.raw,
